 ...
 * Grouping is done by instances of the Group class. GroupLayout supports two types of groups:
 * <table>
 * <tr>
 * <td valign=top>Sequential:
 * <td>A sequential group positions its child elements sequentially, one after another.
 * <tr>
 * <td valign=top>Parallel:
 * <td>A parallel group positions its child elements in the same space on top of each other. Parallel groups can also
 * align the child elements along their baseline.
 * </table>
 * Each Group can contain any number of child groups, Components or gaps. GroupLayout treats each axis independently.
 * That is, there is a group representing the horizontal axis, and a separate group representing the vertical axis. The
 * horizontal group is responsible for setting the x and width of its contents, where as the vertical group is
 * responsible for setting the y and height of its contents.
 * <p>
 * The following code builds a simple layout consisting of two labels in one column, followed by two textfields in the
 * next column:
 * 
 * <pre>
 *   JComponent panel = ...;
 ...
 * <ul>
 * <li>The horizontal axis consists of a sequential group containing two parallel groups. The first parallel group
 * consists of the labels, with the second parallel group consisting of the text fields.
 * <li>The vertical axis similarly consists of a sequential group containing two parallel groups. The parallel groups
 * align their contents along the baseline. The first parallel group consists of the first label and text field, and the
 * second group consists of the second label and text field.
 * </ul>
 * There are a couple of things to notice in this code:
 * <ul>
 * <li>You need not explicitly add the components to the container, this is indirectly done by using one of the
 * <code>add</code> methods.
 * <li>The various <code>add</code> methods of <code>Groups</code> return themselves. This allows for easy chaining of
 * invocations. For example, <code>group.add(label1).add(label2);</code> is equivalent to
 * <code>group.add(label1);group.add(label2);</code>.
 * <li>There are no public constructors for the Groups, instead use the create methods of <code>GroupLayout</code>.
 * </ul>
 * GroupLayout offer the ability to automatically insert the appropriate gap between components. This can be turned on
 * using the <code>setAutocreateGaps()</code> method. Similarly you can use the
 * <code>setAutocreateContainerGaps()</code> method to insert gaps between the components and the container.
 * 
...
public class GroupLayout implements LayoutManager2 {
    ...
    /**
     * Possible argument when linking sizes of components. Specifies the the two component should share the same size
    ...
    /**
     * Possible alignment type. Indicates the elements should be aligned to the origin. For the horizontal axis with a
     * left to right orientation this means aligned to the left.
     *
     * @see #createParallelGroup(int)
     */
    public static final int LEADING = 1;
    /**
     * Possible alignment type. Indicates the elements should be aligned to the end. For the horizontal axis with a left
     * to right orientation this means aligned to the right.
     *
     * @see #createParallelGroup(int)
     */
    ...
    /**
     * Group responsible for layout along the horizontal axis. This is NOT the user specified group, use
     * getHorizontalGroup to dig that out.
     */
    private Group horizontalGroup;
    /**
     * Group responsible for layout along the vertical axis. This is NOT the user specified group, use getVerticalGroup
     * to dig that out.
     */
    ...
    /**
     * Sets whether component visiblity is considered when sizing and positioning components. A value of
     * <code>true</code> indicates that non-visible components should not be treated as part of the layout. A value of
     * <code>false</code> indicates that components should be positioned and sized regardless of visibility.
     * <p>
     * A value of <code>false</code> is useful when the visibility of components is dynamically adjusted and you don't
     ...
     *
     * @return whether component visiblity is considered when sizing and positioning components
     */
    ...
    /**
     * Sets whether the component's visiblity is considered for sizing and positioning. A value of
     * <code>Boolean.TRUE</code> indicates that if <code>component</code> is not visible it should not be treated as
     * part of the layout. A value of <code>false</code> indicates that <code>component</code> is positioned and sized
     * regardless of it's visibility. A value of <code>null</code> indicates the value specified by the single argument
     * method <code>
     * setHonorsVisibility</code> should be used.
     * <p>
     * If <code>component</code> is not a child of the <code>Container</code> this <code>GroupLayout</code> is managing,
     * it will be added to the <code>Container</code>.
     *
     * @param component
     *            the component
     * @param honorsVisibility
     *            whether <code>component</code>'s visiblity should be considered for sizing and positioning
     * @throws IllegalArgumentException
    ...
    private void dump(StringBuffer buffer, Spring spring, String indent, int axis) {
        ...
        buffer.append(indent + spring.getClass().getName() + " " + Integer.toHexString(spring.hashCode()) + " "
                + origin + ", size=" + spring.getSize() + ", alignment=" + spring.getAlignment() + " prefs=["
    ...
    /**
     * Sets whether or not a gap between components should automatically be created. For example, if this is true and
     * you add two components to a <code>SequentialGroup</code> a gap between the two will automatically be created. The
     * default is false.
     *
    ...
    /**
     * Sets whether or not gaps between the container and the first/last components should automatically be created. The
     * default is false.
     *
     * @param autocreatePadding
     ...
     *
     * @return whether or not the gaps between the container and the first/last components should automatically be
     *         created
     */
    ...
    /**
     * Sets the <code>Group</code> that is responsible for layout along the horizontal axis.
     *
     * @param group
     *            <code>Group</code> responsible for layout along the horizontal axis
     * @throws IllegalArgumentException
     *             if group is null
     */
    ...
    /**
     * Returns the <code>Group</code> that is responsible for layout along the horizontal axis.
     *
     * @return <code>ParallelGroup</code> responsible for layout along the horizontal axis.
     */
    ...
    /**
     * Sets the <code>Group</code> that is responsible for layout along the vertical axis.
     *
     * @param group
    ...
    /**
     * Returns the <code>ParallelGroup</code> that is responsible for layout along the vertical axis.
     *
     * @return <code>ParallelGroup</code> responsible for layout along the vertical axis.
     */
    ...
    /**
     * Wraps the user specified group in a sequential group. If container gaps should be generate the necessary springs
     * are added.
     */
    ...
    /**
     * Creates and returns a <code>ParallelGroup</code> with a <code>LEADING</code> alignment. This is a cover method
     * for the more general <code>createParallelGroup(int)</code> method.
     *
    ...
    /**
     * Creates and returns an <code>ParallelGroup</code>. The alignment specifies how children elements should be
     * positioned when the the parallel group is given more space than necessary. For example, if a ParallelGroup with
     * an alignment of TRAILING is given 100 pixels and a child only needs 50 pixels, the child will be positioned at
     * the position 50.
     *
     * @param alignment
     *            alignment for the elements of the Group, one of <code>LEADING</code>, <code>TRAILING</code>,
     *            <code>CENTER</code> or <code>BASELINE</code>.
     * @throws IllegalArgumentException
     *             if alignment is not one of <code>LEADING</code>, <code>TRAILING</code>, <code>CENTER</code> or
     *             <code>BASELINE</code>
     ...
     *            <code>CENTER</code> or <code>BASELINE</code>.
     * @param resizable
     *            whether or not the group is resizable. If the group is not resizable the min/max size will be the same
     *            as the preferred.
     * @throws IllegalArgumentException
     *             if alignment is not one of <code>LEADING</code>, <code>TRAILING</code>, <code>CENTER</code> or
     *             <code>BASELINE</code>
    ...
    /**
     * Creates and returns a <code>ParallelGroup</code> that aligns it's elements along the baseline.
     *
     * @param resizable
    ...
    /**
     * Forces the set of components to have the same size. This can be used multiple times to force any number of
     ...
     *            Components to force to have same size.
     * @param axis
     *            Axis to bind size, one of HORIZONTAL, VERTICAL or HORIZONTAL | VERTICAL
     * @throws IllegalArgumentException
     *             if <code>components</code> is null, or contains null.
     * @throws IllegalArgumentException
     *             if <code>axis</code> does not contain <code>HORIZONTAL</code> or <code>VERTICAL</code>
     */
     ...
     * @throws IllegalArgumentException
     *             is either of the Components are null or if existingComponent is not being managed by this layout
     *             manager
     */
    ...
    /**
     * Sets the LayoutStyle this GroupLayout is to use. A value of null can be used to indicate the shared instance of
    ...
    //
    /**
     * Notification that a <code>Component</code> has been added to the parent container. Developers should not invoke
     * this method directly, instead you should use one of the <code>Group</code> methods to add a
     * <code>Component</code>.
     *
     * @param name
    ...
    /**
     * Notification that a <code>Component</code> has been removed from the parent container. You should not invoke this
     * method directly, instead invoke <code>remove</code> on the parent <code>Container</code>.
     *
     * @param component
     ...
     * @throws IllegalArgumentException
     *             if <code>parent</code> is not the same <code>Container</code> that this was created with
    ...
    //
    /**
     * Notification that a <code>Component</code> has been added to the parent container. You should not invoke this
     * method directly, instead you should use one of the <code>Group</code> methods to add a <code>Component</code>.
     *
     * @param component
     *            The component added
     * @param constraints
    ...
    /**
     * Adjusts the autopadding springs for the horizontal and vertical groups. If <code>insert</code> is true this will
     * insert auto padding springs, otherwise this will only adjust the springs that comprise auto preferred padding
     * springs.
     */
    ...
    /**
     * Spring consists of a range: min, pref and max a value some where in the middle of that and a location. Subclasses
     * must override methods to get the min/max/pref and will likely want to override the <code>setSize</code> method.
     * Spring automatically caches the min/max/pref. If the min/pref/max has internally changes, or needs to be updated
     * you must invoked clear.
     */
    abstract class Spring {
        ...
        /**
         * Sets the value and location of the spring. Subclasses will want to invoke super, then do any additional
         * sizing.
         *
         * @param axis
        ...
        /**
         * Returns true if this Spring will ALWAYS have a zero size. This should NOT check the current size, rather it's
         * meant to quickly test if this Spring will always have a zero size.
         */
        abstract boolean willHaveZeroSize(boolean treatAutopaddingAsZeroSized);
    }

    /**
     * Group provides for commonality between the two types of operations supported by <code>GroupLayout</code>: laying
     * out components one after another (<code>SequentialGroup</code>) or layout on top of each other (
     * <code>ParallelGroup</code>). Use one of <code>createSequentialGroup</code> or <code>createParallelGroup</code> to
     * create one.
     */
    public abstract class Group extends Spring {
        ...
        /**
         * Used to compute how the two values representing two springs will be combined. For example, a group that layed
         * things out one after the next would return <code>a + b</code>.
         */
        abstract int operator(int a, int b);

        /**
         * Calculates the specified size. This is called from one of the <code>getMinimumSize0</code>,
         * <code>getPreferredSize0</code> or <code>getMaximumSize0</code> methods. This will invoke to
        ...
        // Padding
        /**
         * Adjusts the autopadding springs in this group and its children. If <code>insert</code> is true this will
         * insert auto padding springs, otherwise this will only adjust the springs that comprise auto preferred padding
         * springs.
         *
         * @param axis
         ...
         * @param insert
         *            Whether or not to insert AutopaddingSprings or just adjust any existing AutopaddingSprings.
         */
    ...
    /**
     * A <code>Group</code> that lays out its elements sequentially, one after another. This class has no public
     * constructor, use the <code>createSequentialGroup</code> method to create one.
     *
    ...
    public class SequentialGroup extends Group {
         ...
         * @param useAsBaseline
         *            whether the specified <code>Group</code> should be used to calculate the baseline for this
         *            <code>Group</code>
        ...
        /**
         * Adds the specified Component. If the Component's min/max are different from its pref than the component will
         * be resizable.
         *
         * @param component
         ...
         *
         * @param useAsBaseline
         *            whether the specified <code>Component</code> should be used to calculate the baseline for this
         *            <code>Group</code>
         * @param component
        ...
        /**
         * Adds the specified <code>Component</code>. Min, pref and max can be absolute values, or they can be one of
         * <code>DEFAULT_SIZE</code> or <code>PREFERRED_SIZE</code>. For example, the following:
         * 
         * <pre>
         * add(component, PREFERRED_SIZE, PREFERRED_SIZE, 1000);
         * </pre>
         * 
         * Forces a max of 1000, with the min and preferred equalling that of the preferred size of
         * <code>component</code>.
         *
         * @param component
         *            the Component to add
        ...
        /**
         * Adds a <code>Component</code> to this <code>Group</code> with the specified size.
         *
         * @param useAsBaseline
         *            whether the specified <code>Component</code> should be used to calculate the baseline for this
         *            <code>Group</code>
         ...
         *            the <code>Component</code> to add
         * @param min
         *            the minimum size or one of <code>DEFAULT_SIZE</code> or <code>PREFERRED_SIZE</code>
         * @param pref
         *            the preferred size or one of <code>DEFAULT_SIZE</code> or <code>PREFERRED_SIZE</code>
         * @param max
         *            the maximum size or one of <code>DEFAULT_SIZE</code> or <code>PREFERRED_SIZE</code>
         * @return this <code>Group</code>
         */
        ...
        /**
         * Adds an element representing the preferred gap between the nearest components. That is, during layout the
         * neighboring components are found, and the min, pref and max of this element is set based on the preferred gap
         * between the components. If no neighboring components are found the min, pref and max are set to 0.
         * 
         * @param type
         *            the type of gap; one of the LayoutStyle constants
         * @return this SequentialGroup
         * @throws IllegalArgumentException
         *             if type is not one of <code>LayoutStyle.RELATED</code> or <code>LayoutStyle.UNRELATED</code>
         * @see LayoutStyle
         */
        ...
        /**
         * Adds an element for the preferred gap between the nearest components. That is, during layout the neighboring
         * components are found, and the min of this element is set based on the preferred gap between the components.
         * If no neighboring components are found the min is set to 0. This method allows you to specify the preferred
         * and maximum size by way of the <code>pref</code> and <code>max</code> arguments. These can either be a value
         * &gt;= 0, in which case the preferred or max is the max of the argument and the preferred gap, of
         * DEFAULT_VALUE in which case the value is the same as the preferred gap.
         * 
         * @param type
         *            the type of gap; one of LayoutStyle.RELATED or LayoutStyle.UNRELATED
         * @param pref
         *            the preferred size; one of DEFAULT_SIZE or a value > 0
         * @param max
         *            the maximum size; one of DEFAULT_SIZE, PREFERRED_SIZE or a value > 0
         * @return this SequentialGroup
         * @throws IllegalArgumentException
         *             if type is not one of <code>LayoutStyle.RELATED</code> or <code>LayoutStyle.UNRELATED</code> or
         *             pref/max is != DEFAULT_SIZE and < 0, or pref > max
         * @see LayoutStyle
         */
        ...
        /**
         * Adds an element representing the preferred gap between one edge of the container and the next/previous
         * Component. This will have no effect if the next/previous element is not a Component and does not touch one
    ...
    /**
     * A <code>Group</code> that lays out its elements on top of each other. If a child element is smaller than the
     * provided space it is aligned based on the alignment of the child (if specified) or on the alignment of the
     * ParallelGroup.
     *
    ...
    public class ParallelGroup extends Group {
         ...
         * @return this <code>ParallelGroup</code>
         * @throws IllegalArgumentException
         *             if alignment is not one of <code>LEADING</code>, <code>TRAILING</code> or <code>CENTER</code>
         */
        ...
        void insertAutopadding(int axis, List leadingPadding, List trailingPadding, List leading, List trailing,
            ...
            for (int counter = 0, max = springs.size(); counter < max; counter++) {
                ...
                if (spring instanceof ComponentSpring) {
                    ...
                    ((Group) spring)
                            .insertAutopadding(axis, leadingPadding, trailingPadding, leading, trailing, insert);
    ...
    /**
     * An extension of <code>ParallelGroup</code> that aligns its constituent <code>Spring</code>s along the baseline.
     */
    ...
    /**
     * A Spring representing one axis of a Component. There are three ways to configure this:
     * <ul>
     * <li>Use the pref/min/max from the component
     * <li>Use the pref from the component and fix the min to 0 or max to a big number.
     * <li>Force the min/max/pref to be a certain value. If the Component's size is to be linked to another components
     * than the min/max/pref all come from the ComponentInfo.
     */
    ...
    /**
     * Spring reprensenting the distance between any number of sources and targets. The targets and sources are computed
     * during layout. An instance of this can either be dynamically created when autocreatePadding is true, or
    ...
    /**
     * Tracks the horizontal/vertical Springs for a Component. This class is also used to handle Springs that have their
     * sizes linked.
     */
