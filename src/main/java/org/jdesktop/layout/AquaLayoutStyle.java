...
class AquaLayoutStyle extends LayoutStyle {
    ...
    /**
     * The containerGapDefinitions array defines the preferred insets (child gaps) of a parent container towards one of
     * its child components.
     *
     * Note: As of now, we do not yet specify the preferred gap from a child to its parent. Therefore we may not be able
     * to treat all special cases.
     *
     * This array is used to initialize the containerGaps HashMap.
     *
     * The array has the following structure, which is supposed to be a a compromise between legibility and code size.
     * containerGapDefinitions[0..n] = preferred insets for some parent UI's containerGapDefinitions[][0..m-3] = name of
     * parent UI, optionally followed by a full stop and a style name containerGapDefinitions[][m-2] = mini insets
     * containerGapDefinitions[][m-1] = small insets containerGapDefinitions[][m] = regular insets
     */
    ...
    /**
     * The relatedGapDefinitions table defines the preferred gaps of one party of two related components.
     *
     * The effective preferred gap is the maximum of the preferred gaps of both parties.
     *
     ...
     * containerGapDefinitions[][0..m-3] = name of UI optionally followed by a full stop and a style name
     * containerGapDefinitions[][m-2] = mini insets containerGapDefinitions[][m-1] = small insets
    ...
    /**
     * The indentGapDefinitions table defines the preferred indentation for components that are indented after the
     * specified component.
     *
     * This array is used to initialize the indentGaps HashMap.
     *
     * The array has the following structure, which is supposed to be a a compromise between legibility and code size.
     * indentGapDefinitions[0..n] = preferred gaps for a party of a two related UI's indentGapDefinitions[][0..m-3] =
     * name of UI optionally followed by a full stop and a style name indentGapDefinitions[][m-2] = mini insets
     * indentGapDefinitions[][m-1] = small insets indentGapDefinitions[][m] = regular insets
     */
    ...
    /**
     * The visualMarginDefinition table defines the visually perceived margin of the components.
     *
     ...
     * visualMarginDefinitions[][0..m-1] = name of UI optionally followed by a full stop and a style name
    ...
    /**
     * The relatedGaps map defines the preferred gaps of one party of two related components.
     */
    private final static Map RELATED_GAPS = createInsetsMap(relatedGapDefinitions);
    /**
     * The unrelatedGaps map defines the preferred gaps of one party of two unrelated components.
     */
    private final static Map UNRELATED_GAPS = createInsetsMap(unrelatedGapDefinitions);
    /**
     * The containerGaps map defines the preferred insets (child gaps) of a parent component towards one of its
     * children.
     */
    private final static Map CONTAINER_GAPS = createInsetsMap(containerGapDefinitions);
    /**
     * The indentGaps map defines the preferred indentation for components that are indented after the specified
     * component.
     */
    private final static Map INDENT_GAPS = createInsetsMap(indentGapDefinitions);
    /**
     * The visualMargins map defines the preferred indentation for components that are indented after the specified
     * component.
     */
     ...
     * ComponentInsets may have sub styles.
     */
     ...
     * <code>component2</code> relative to <code>component1</code>. For example, the following returns the amount of
     * space to place between <code>component2</code> and <code>component1</code> when <code>component2</code> is placed
     ...
     * </pre>
     * 
     * The <code>type</code> parameter indicates the type of gap being requested. It can be one of the following values:
     * <table>
     * <tr>
     * <td><code>RELATED</code>
     * <td>If the two components will be contained in the same parent and are showing similar logically related items,
     * use <code>RELATED</code>.
     * <tr>
     * <td><code>UNRELATED</code>
     * <td>If the two components will be contained in the same parent but show logically unrelated items use
     * <code>UNRELATED</code>.
     * <tr>
     * <td><code>INDENT</code>
     * <td>Used to obtain the preferred distance to indent a component relative to another. For example, if you want to
     * horizontally indent a JCheckBox relative to a JLabel use <code>INDENT</code>. This is only useful for the
     * horizontal axis.
     * </table>
     * <p>
     * It's important to note that some look and feels may not distinguish between <code>RELATED</code> and
     * <code>UNRELATED</code>.
     * <p>
     * The return value is not intended to take into account the current size and position of <code>component2</code> or
     * <code>component1</code>. The return value may take into consideration various properties of the components. For
     * example, the space may vary based on font size, or the preferred size of the component.
     *
     * @param component1
     *            the <code>JComponent</code> <code>component2</code> is being placed relative to
     * @param component2
     *            the <code>JComponent</code> being placed
     * @param type
     *            how the two components are being placed
     * @param position
     *            the position <code>component2</code> is being placed relative to <code>component1</code>; one of
     *            <code>SwingConstants.NORTH</code>, <code>SwingConstants.SOUTH</code>, <code>SwingConstants.EAST</code>
     *            or <code>SwingConstants.WEST</code>
     * @param parent
     *            the parent of <code>component2</code>; this may differ from the actual parent and may be null
     ...
     *             <code>RELATED</code> or <code>UNRELATED</code>; or <code>component1</code> or <code>component2</code>
     *             is null
     */
     ...
     *            or <code>SwingConstants.WEST</code>
     * @param parent
     *            the parent of <code>component</code>; this may differ from the actual parent and may be null
    ...
    /**
     * ComponentInsets is used to manage the Insets for a specific Component type. Each ComponentInsets may also have
     * children (sub) ComponentInsets. Subinsets are used to represent different styles a component may have. For
     * example, a Button may not a set of insets, as well as insets when it has a style of metal.
     */
